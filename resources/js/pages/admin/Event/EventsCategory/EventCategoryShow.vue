  ...
  <section class="mt-3">
    <div class="card">
      <div class="card-body" v-if="category">
        ...
        <router-link :to="{ name: 'EventCategoryIndex' }" class="btn btn-secondary mt-3">Back to List</router-link>
        <router-link v-if="authStore.hasPermission('edit-events-categories')"
          :to="{ name: 'UpdateEventCategory', params: { id: category?.id } }" class="ml-2 btn btn-primary mt-3 ms-2">
          Edit
        </router-link>
      </div>
          ...
          <table class="table table-bordered table-hover">
            ...
            <tbody>
              <tr v-for="(cat, index) in category?.children" :key="cat.id">
                ...
                <td class="align-middle">
                  <div class="d-flex">
                    <button v-if="authStore.hasPermission('view-events-categories')" @click="fetchCategory(cat.id)"
                      class="btn btn-sm btn-dark">
                      <i class="fas fa-eye"></i>
                    </button>

                    <router-link v-if="authStore.hasPermission('edit-events-categories')"
                      :to="{ name: 'UpdateEventCategory', params: { id: cat.id } }" class="ml-2 btn btn-sm btn-dark">
                      <i class="fas fa-pencil-alt"></i>
                    </router-link>

                    <button v-if="authStore.hasPermission('delete-events-categories')" class="ml-2 btn btn-sm btn-danger"
