<template>
    <DashboardHeader title="Event Category Details" :back="true" @back="goBack()" />
    ...
    <section class="mt-3">
        <div class="card">
            <div class="card-body" v-if="category">
                ...
                <router-link :to="{ name: 'EventCategoryIndex' }" class="btn btn-secondary mt-3">Back to
                    List</router-link>
                <router-link v-if="authStore.hasPermission('edit-events-categories')"
                    :to="{ name: 'UpdateEventCategory', params: { id: category?.id } }"
                    class="ml-2 btn btn-primary mt-3 ms-2">
                    Edit
                </router-link>
            </div>
                        ...
                        <table class="table table-bordered table-hover">
                            <thead class="thead-light">
                                <tr class="align-middle">
                                    ...
                                    <th>Parent</th>
                                    <th>Description</th>
                                    <th v-if="authStore.hasPermission('edit-events-categories')">Status</th>
                                    <th>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(cat, index) in category?.children" :key="cat.id">
                                    ...
                                    <!-- Status dropdown -->
                                    <td v-if="authStore.hasPermission('edit-events-categories')" class="align-middle">
                                    ...
                                    <td class="align-middle">
                                        <div class="d-flex">
                                            <button v-if="authStore.hasPermission('view-events-categories')"
                                                @click="fetchCategory(cat.id)" class="btn btn-sm btn-dark">
                                                <i class="fas fa-eye"></i>
                                            </button>

                                            <router-link v-if="authStore.hasPermission('edit-events-categories')"
                                                :to="{ name: 'UpdateEventCategory', params: { id: cat.id } }"
                                                class="ml-2 btn btn-sm btn-dark">
                                                <i class="fas fa-pencil-alt"></i>
                                            </router-link>

                                            <button v-if="authStore.hasPermission('delete-events-categories')"
                                                class="ml-2 btn btn-sm btn-danger" @click="confirmDelete(cat)">
...
// Current route instance (to access category ID from params)
const route = useRoute();

// Single event category data (fetched from API)
...
/**
 * Fetch the event category details by ID
 * @param {number|string} id - Category ID (defaults to route.params.id)
 */
const fetchCategory = async (id = route.params.id) => {
    try {
        const res = await axios.get(`/api/events-categories/${id}`);
        category.value = res.data.data; // Expected: { category_name, description, image, parent, ... }
    } catch (error) {
...
// Optional props (kept for flexibility, though ID is primarily from route)
defineProps({
    id: {
        type: [Number, String],
        required: false
