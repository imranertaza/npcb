<template>
    <DashboardHeader title="Manage Event Categories">
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                                <tr v-for="(cat, index) in categories?.data" :key="cat.id">
                                    ...
                                    <td class="align-middle">
                                        <div class="d-flex">
                                            <router-link v-if="authStore.hasPermission('view-events-categories')"
                                                :to="{ name: 'EventCategoryShow', params: { id: cat.id } }"
...
// Reactive list of event categories (paginated response)
...
const updateStatus = async (cat) => {
    try {
        await axios.patch(`/api/events-categories/${cat.id}`, {
            status: cat.status
        });
...
/**
 * Confirm and delete an event category
 */
const confirmDelete = async (cat) => {
    ...
    if (result?.isConfirmed) {
        try {
            await axios.delete(`/api/events-categories/${cat.id}`);
            toast.success('Category deleted successfully!');

            // Refresh current page (preserves pagination position)
