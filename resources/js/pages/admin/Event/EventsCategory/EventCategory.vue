                  ...
                  <td class="align-middle">
                    <div class="d-flex">
                      <router-link v-if="authStore.hasPermission('view-events-categories')"
                        :to="{ name: 'EventCategoryShow', params: { id: cat.id } }" class="btn btn-sm btn-outline-dark">
                        <i class="fas fa-eye"></i>
                      </router-link>

                      <router-link v-if="authStore.hasPermission('edit-events-categories')"
                        :to="{ name: 'UpdateEventCategory', params: { id: cat.id } }" class="ml-2 btn btn-sm btn-outline-info">
                        <i class="fas fa-pencil-alt"></i>
                      </router-link>

                      <button v-if="authStore.hasPermission('delete-events-categories')" class="ml-2 btn btn-sm btn-outline-danger"
...
const updateStatus = async (cat) => {
  try {
    const response = await axios.patch(`/api/events-categories/${cat.id}`, {
      status: cat.status,
    });
    if (cat.status == 1) {
