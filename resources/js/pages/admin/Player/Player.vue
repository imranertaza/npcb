              ...
              <tbody>
                <tr v-for="(player, index) in players?.data" :key="player.id">
                  ...
                  <td class="align-middle">
                    <button v-if="player.image" @click="openImageModal(player.image)" class="text-nowrap btn btn-sm btn-outline-primary">
                      View Image
                    </button>
                  </td>
                  <td v-if="authStore.hasPermission('edit-players')" class="align-middle">
                    <select v-model="player.status" @change="updateStatus(player)" class="custom-select"
